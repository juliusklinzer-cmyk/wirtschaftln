...
export async function gruenden(token: string, _prev: GruendungsState, formData: FormData): Promise<GruendungsState> {
  ...
  const eingabe: GruendungsEingabe = {
    ...
    typ: feld('typ'),
    stammhausName: String(formData.get('stammhaus_name') ?? '') || String(formData.get('stammhaus_freitext') ?? ''),
    stammhausAdresse: String(formData.get('stammhaus_adresse') ?? ''),
    stammhausBezirk: String(formData.get('stammhaus_bezirk') ?? ''),
    stammhausTelefon: String(formData.get('stammhaus_telefon') ?? ''),
    stammhausLat: String(formData.get('stammhaus_lat') ?? ''),
    stammhausLng: String(formData.get('stammhaus_lng') ?? ''),
