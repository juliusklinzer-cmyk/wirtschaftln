...
const ASSETS = ['/brand/shield-256.png', '/brand/shield-512.png', '/brand/rauten-256.png', '/brand/rauten-512.png'];
...
self.addEventListener('push', (event) => {
  ...
  event.waitUntil(
    self.registration.showNotification(data.title || 'Wirtschaftln', {
      body: data.body || '',
      icon: data.icon || '/brand/rauten-256.png',
      badge: data.icon || '/brand/rauten-256.png',
      data: { url: data.url || '/' },
    })
  );
});
