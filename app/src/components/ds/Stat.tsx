...
export function Stat({
  ...
  tone?: 'blau' | 'gold' | 'dark' | 'light' | 'ink';
  align?: 'left' | 'center';
  size?: 'sm' | 'md' | 'lg';
  style?: React.CSSProperties;
}) {
  const colors = {
    blau: 'var(--muc-blau)', gold: 'var(--gold-700)', dark: 'var(--navy)',
    light: 'var(--weiss)', ink: 'var(--ink-900)',
  };
  ...
  return (
    <div style={{ textAlign: align, fontFamily: 'var(--font-ui)', ...style }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, justifyContent: align === 'center' ? 'center' : 'flex-start' }}>
        ...
        <span
          style={{
            fontSize: sizes[size], fontWeight: 800, letterSpacing: '-0.03em',
            color: numColor, fontVariantNumeric: 'tabular-nums', lineHeight: 1,
          }}
        >
          {value}
        </span>
      ...
      <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: labColor, marginTop: 5 }}>
        {label}
      </div>
    </div>
  );
